...
conn.commit()
account = st.sidebar.selectbox("Create Account or Log in",["Create Account","Log in"])
if account == "Log in" :
    username = st.sidebar.text_input("Input Username", "Name")
    password = st.sidebar.text_input("Input password", "password")
    ...
    if st.sidebar.button("Submit"):
        ...
        if auth :
            ...
            if st.sidebar.button("Logout") == True :
                st.write("Logged Out Successfully")
        else :
            st.success("You Have no account here, create a new account")
            st.header("Username or Password not correct")
else :
    
    username = st.sidebar.text_input("Type Username")
